...
describe('Api', () => {
  ...
  describe('POST /api/games/:id/guesses', () => {
     ...
     beforeAll(async () => {
        const gameSettings = { playerId: 'GuessTester', length: 5, unique: false, lang: 'english' };
        const startResponse = await request(app).post('/api/games').send(gameSettings);
     ...
     it('should accept a valid guess and return results', async () => {
        ...
        const response = await request(app).post(`/api/games/${gameId}/guesses`).send(guessData);
